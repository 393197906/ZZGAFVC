...
import com.massclouds.ns.dao.uitl.HqlFilter;
...
public class BaseServiceImpl<T> implements BaseService<T> {
	...
	@Autowired
	BaseDao<UserModel> userModel;
	
	protected int[] perArr  = new int[]{0,1,2};  //权限数组
	//权限管理
	protected boolean hasPermission(String user){
		String hql = " from UserModel where username='"+user+"'";
		UserModel um = userModel.getByHql(hql);
		Integer permission = um.getPermission();
		for(int i =0;i<perArr.length;i++){
			if(permission ==perArr[i] ){
				return true;
			}
		}
		return false;
