...
$(function () {
	...
	if (-1 == WebVideoCtrl.I_CheckPluginInstall()) {
		$.messager.confirm('提示','您还未安装过插件，是否安装！',function(r){    
		    if (r){    
		    	location.href = "/ns/software/WebComponents.exe";
		    }    
		});  
	...
	var $divPlugin = $("#divPlugin");

	// 初始化插件参数及插入插件
	WebVideoCtrl.I_InitPlugin($divPlugin.parent().width(), $divPlugin.parent().height(), {
        iWndowType: 1,
...
function loginStart(camera) {
	...
	var szIP = camera.ip,
		...
		szPassword = camera.password;
	var iRet = WebVideoCtrl.I_Login(szIP, 1, szPort, szUsername, szPassword, {
		success: function (xmlDoc) {
			//alert(szIP + " 登录成功！");
			setTimeout(function(){
				setCookie("cip",szIP);
				clickStartRealPlay({ip:szIP,streamType:camera.streamType}); //开始预览
			},500)
//			setCookie("cip",szIP);
			//clickStartRealPlay({ip:szIP,streamType:camera.streamType}); //开始预览
		},
...
function clickStartRealPlay(info) {
	...
	var iRet = WebVideoCtrl.I_StartRealPlay(info.ip, {
		iStreamType: info.streamType,  //码流
		iChannelID:1,
		bZeroChannel: false
	});
	if (0 == iRet) {
